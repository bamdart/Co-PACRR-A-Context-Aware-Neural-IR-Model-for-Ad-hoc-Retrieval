...
def _load_doc_mat_desc(qids, qid_topic_idf, qid_desc_idf):
    ...
    for qid in sorted(qids):
        ...
        for cwid in cwid_list: ##############  這邊要改成讀取資料夾內的所有npy
            label = int(cwid[-5])
            cwid = cwid[:-4]
            qid_cwid_label[qid][cwid[:-2]] = label
            ...
            if usetopic and usedesc and topic_mat.shape[1] != desc_mat.shape[1]:
                empty = True
            if not empty:
                qid_cwid_simmat[qid][cwid[:-2]] = np.concatenate(m, axis=0).astype(np.float32)

    return qid_cwid_simmat, qid_term_idf, qid_cwid_label
...
def sample_train_data_weighted(qid_wlen_cwid_mat, qid_cwid_label, query_idfs, sample_qids):
    ...
    sample_label_prob = {0:0.5,1:0.5}#這裡我們應該只有1
    random_seed = 14
    n_batch = 32
    NUM_NEG = 10
...
def load_training_data():
    ...
    qid_cwid_rmat, qid_term_idf, qid_cwid_label = _load_doc_mat_desc(qids, qid_topic_idf, qid_desc_idf) # 讀取doc的npy
