...
'''
load_train_data_generator
 @param     qids                get_train_qids(p['train_years'])
 @param     rawdoc_mat_dir      the directory helds the similarity matrices
 @param     qid_cwid_label      read_qrel(qrelf, qids, include_spam=False)
 @param     N_GRAMS             get_ngram_nfilter(p['winlen'], p['qproximity'], p['maxqlen'], p['xfilters'])
 @param     param_val           default_params
 @param     label2tlabel
 @param     sample_label_prob
'''
def load_train_data_generator(qids, rawdoc_mat_dir, qid_cwid_label, N_GRAMS, param_val, label2tlabel={4:2,3:2,2:2,1:1,0:0,-2:0}, sample_label_prob={2:0.5,1:0.5}):
    POS_METHOD = param_val['distill']
    SIM_DIM = param_val['simdim']
    NUM_NEG = param_val['numneg']
    MAX_QUERY_LENGTH = param_val['maxqlen'] 
